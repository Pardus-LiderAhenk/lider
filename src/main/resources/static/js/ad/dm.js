				...
				if (treeMenuSelection.attributesMultiValues.hasOwnProperty(key) ) {
					for(var i = 0; i< treeMenuSelection.attributesMultiValues[key].length; i++) {
						if(key != 'objectGUID' 
							...
							&& key !='replUpToDateVector'
							&& key !='msDS-IsDomainFor'
							&& key !='maxPwdAge'
							&& key !='rIDManagerReference'
							&& key !='otherWellKnownObjects'
							&& key !='lockoutDuration'
...
function createDmTreeGrid() {
	...
	progress(treeGridHolderDiv,"progressAdTree", 'show')
	...
	createDMTree(treeGridHolderDiv, false, false,
			...
			function(rootDN, treeGridId,firstRow, error){
				...
				if(error != null ){
					...
					$('#treeMenu').hide();
					progress(treeGridHolderDiv,"progressAdTree",'hide')
				}else{
